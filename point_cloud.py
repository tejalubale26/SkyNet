...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def sendHoverCommand(self):
        '''Send the hover command to the Crazyflie'''
        self.cf.commander.send_hover_setpoint(
            self.hover['x'], self.hover['y'], self.hover['yaw'],
            self.hover['height'])

    def updateHover(self, k, v):
        '''Update the hover command with the new value from the keypress'''
        if (k != 'height'):
    ...
    def connected(self, URI):
        '''This is called when the Crazyflie is connected'''
        ...
        try:
            ...
            self.cf.log.add_config(lpos)                    # Add the configuration to the logger
            lpos.data_received_cb.add_callback(self.pos_data)       # Add the callback
            lpos.start()                                        # Start the logging
    ...
    def pos_data(self, timestamp, data, logconf):
        '''This is called when new data arrives from the Crazyflie'''
        position = [
...
class Canvas(scene.SceneCanvas):
    def __init__(self, keyupdateCB):
        ...
        self.size = 800, 600                            # Set the size of the canvas
        self.unfreeze()                                 # Allow the canvas to be updated
        self.view = self.central_widget.add_view()      # Add a view to the canvas
        self.view.bgcolor = '#ffffff'                   # Set the background color of the canvas
        self.view.camera = TurntableCamera(             # Set the camera type
            fov=10.0, 
            distance=30.0, 
            up='+z', 
            center=(0.0, 0.0, 0.0))
        self.last_pos = [0, 0, 0]
        self.pos_markers = visuals.Markers()             # Create a marker for the position
        self.meas_markers = visuals.Markers()            # Create a marker for the measurements
        self.pos_data = np.array([0, 0, 0], ndmin=2)     # Set the position data
        self.meas_data = np.array([0, 0, 0], ndmin=2)    # Set the measurement data
        self.lines = []
    ...
    def set_position(self, pos):
        '''Set the position of the Crazyflie and update the plot'''
        self.last_pos = pos
        if (PLOT_CF):
    ...
    def rot(self, roll, pitch, yaw, origin, point):
        '''Rotate a point around an origin in 3D space'''
        ...
        # Create the rotation matrix for the roll, pitch and yaw
    ...
    def rotate_and_create_points(self, m):
        '''Rotate the sensor measurements and create the points in 3D space'''
        data = []
        o = self.last_pos
        roll = m['roll']
        pitch = -m['pitch']
    ...
    def set_measurement(self, measurements):
        '''Set the measurements and update the plot'''

...
class CrazyflieServer(Node):
    def __init__(self):
        ...
        # Conver ROS parameters into a dictionary 
        ...
        # Load the Crazyflie parameters from the ROS parameters
        self.world_tf_frame = "world"   
        try:
            self.world_tf_frame = self._ros_parameters["world_tf_frame"]
        except KeyError:
            pass
        ...
        #Initialize logging, servics and parameters for each crazyflie
        ...
        try:
            ...
            self.get_logger().error("Error!: One or more Crazyflies can not be found. ")
            self.get_logger().error("Check if you got the right URIs, if they are turned on" +
        ...
        # Create services and subscriptions for each Crazyflie
            self.create_service(
                Empty, name +
                "/emergency", 
                partial(self._emergency_callback, uri=uri)
            )
            self.create_service(
                Takeoff, name +
                "/takeoff", 
                partial(self._takeoff_callback, uri=uri)
            )
            self.create_service(
                Land, name + "/land", 
                partial(self._land_callback, uri=uri)
            )
            self.create_service(
                GoTo, name + "/go_to", 
                partial(self._go_to_callback, uri=uri)
            )
            self.create_service(
            ...
            self.create_subscription(
                Twist, name +
                "/cmd_vel_legacy", 
                partial(self._cmd_vel_legacy_changed,
                                           uri=uri), 10
            )
            self.create_subscription(
                Hover, name +
                "/cmd_hover", 
                partial(self._cmd_hover_changed, uri=uri), 10
            )

            self.create_subscription(
                FullState, name +
                "/cmd_full_state", 
                partial(self._cmd_full_state_changed, uri=uri), 10
            )
    ...
    def _param_to_dict(self, param_ros):
        """
        Turn ROS 2 parameters into a dictionary
        """
        tree = {}
    ...
    def _log_scan_data_callback(self, timestamp, data, logconf, uri):
        ...
        """
        # Get the name of the Crazyflie
        cf_name = self.cf_dict[uri]
        max_range = 3.49

        # Get the range data from the Crazyflie and convert it to meters
        ...
        # If the range is greater than the max range, set it to infinity
        ...
        # Create the LaserScan message and publish it
        msg = LaserScan()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = cf_name
        msg.range_min = 0.01        # 1cm
        msg.range_max = 3.49        # 3.49m
        msg.ranges = self.ranges    # [back, right, front, left]
        msg.angle_min = -0.5 * 2 * pi       
        msg.angle_max = 0.25 * 2 * pi    
        msg.angle_increment = 1.0 * pi/2        # Angle increment is 90 degrees
    ...
    def _log_pose_data_callback(self, timestamp, data, logconf, uri):
        ...
        # Get the pose data from the Crazyflie
